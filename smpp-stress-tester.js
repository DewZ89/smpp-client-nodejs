...
var session = smpp.connect("smpp://jgateway.arolitec.com:2777")
...
const test_sms_count = 500 // Number of SMS messages to send.
const req_sec_limit = 200 // Number of messages per second
  ...
  function (pdu) {
    if (pdu.command_status == 0) {
      ...
      for (i = 0; i < test_sms_count; i++) {
        ...
        session.submit_sm(
          {
            destination_addr: "2250741165280", // To Phone Number
            source_addr: "AROLITEC", // From Number or Sender ID
...
console.log(
  "Sent " +
    ...
    failed_count +
    ". Total req/sec:" +
    success_count / (end_test_ts - start_test_ts) +
    ". Waited for " +
    (end_test_ts - last_sms_sent_time) +
    " sec for last SMS"
)
process.exit()
